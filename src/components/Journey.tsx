...
export default function Journey() {
  ...
  return (
    <section id="experience" className="py-24 relative z-10 border-t border-[rgba(200,61,74,0.15)] bg-[#100406]/60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {milestones.map((item, idx) => (
            <motion.div
              ...
              className="reference-card p-8 flex flex-col justify-between space-y-6 relative overflow-hidden group"
            >
              <div className="flex items-center justify-between border-b border-[rgba(200,61,74,0.2)] pb-4">
                <span className="font-display font-black text-4xl text-[#f7e9e1] group-hover:text-[#c83d4a] transition-colors">
                  {item.num}
                </span>
                <div className="p-3 rounded-2xl bg-[#120608] border border-[rgba(200,61,74,0.3)] shadow-inner">
                  {item.icon}
                </div>
              </div>

              <div className="space-y-2 my-auto">
                <h3 className="font-display font-extrabold text-2xl text-[#f7e9e1] uppercase tracking-wider">
                  {item.title}
                </h3>
              ...
              <div className="text-[10px] font-grotesk font-bold text-[#c83d4a] tracking-widest uppercase border-t border-[rgba(200,61,74,0.2)] pt-3">
                MILESTONE 0{idx + 1}
              </div>
            </motion.div>
          ))}
