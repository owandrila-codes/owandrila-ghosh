...
export default function Skills() {
  ...
  const categories: SkillCategory[] = [
    {
      title: 'PROGRAMMING',
      icon: <Code2 className="w-5 h-5 text-[#c83d4a]" />,
      skills: ['C', 'Java', 'JavaScript'],
    },
    {
      title: 'WEB DEVELOPMENT',
      icon: <Globe className="w-5 h-5 text-[#c83d4a]" />,
      skills: ['HTML', 'CSS', 'JavaScript', 'Node.js'],
    },
    {
      title: 'DATA & AI',
      icon: <Brain className="w-5 h-5 text-[#c83d4a]" />,
      skills: ['Data Science', 'Artificial Intelligence', 'Data Analysis', 'AI-assisted Development'],
    },
    {
      title: 'CLOUD & DATABASES',
      icon: <Cloud className="w-5 h-5 text-[#c83d4a]" />,
  ...
  return (
    <section id="skills" className="py-24 relative z-10 border-t border-[rgba(200,61,74,0.15)]">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-grotesk font-bold uppercase tracking-wider transition-all ${
                  activeTab === tab
                    ? 'bg-[#c83d4a] text-[#f7e9e1] border border-[#c83d4a]/40 shadow-md'
                    : 'bg-[#170a0d] text-[#f7e9e1]/70 border border-[rgba(200,61,74,0.2)] hover:border-[#c83d4a] hover:text-[#f7e9e1]'
                }`}
              >
                {tab}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredCategories.map((cat, idx) => (
            <motion.div
              ...
              className="reference-card p-8 space-y-6"
            >
              <div className="flex items-center justify-between border-b border-[rgba(200,61,74,0.2)] pb-4">
              ...
              <div className="flex flex-wrap gap-2.5">
                {cat.skills.map((skill) => (
                  <div
                    key={skill}
                    className="px-4 py-2.5 rounded-2xl bg-[#120608]/90 border border-[rgba(200,61,74,0.25)] text-xs font-grotesk font-bold text-[#f7e9e1] hover:border-[#c83d4a] hover:bg-[#8b1e27]/20 transition-all flex items-center gap-2"
                  >
